/*      OBJETIVOS
    ...
    - Mostrar que o resultado é POSITIVO ou NEGATIVO utilzando TEMPLATE STRING.
...
function calculating() {
    ...
    switch (choice) {
        case 'average':
            ...
            pResult.innerHTML = `Número ${type} igual á ${parseInt(result)}`
            break;

        case 'addition':
            result = Numcber(input0.value) + Number(input1.value)
            if (result < 0) {
                type = 'NEGATIVO'
            } else {
                type = 'POSITIVO'
